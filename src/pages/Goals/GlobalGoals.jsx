// src/pages/Goals/GlobalGoals.jsx
...
const GlobalGoals = () => {
  ...
  const fetchGoalsAndBacklinks = async (projectId) => {
    ...
    const dueSoon = goalData.filter(
      (g) => g.dueDate <= today && total < g.target
    );
  ...
  return (
    <div className="project-goals-page">
      <h2>📊 Project Goals</h2>
        ...
        <div className="notifications-box">
          ...
          {showNotifications && (
            <ul className="reminder-list">
              {reminders.map((g) => (
                <li key={g.id}>
                  <strong>{g.title}</strong> is due on{" "}
                  <span>{g.dueDate}</span>
                </li>
              ))}
            </ul>
